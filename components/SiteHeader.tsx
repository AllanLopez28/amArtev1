...
export default function SiteHeader({
  ...
  return (
    <>
      ...
      {/* Menú móvil: PANEL con fondo BLANCO sólido */}
      {open && (
        <>
          {/* overlay */}
          <div
          ...
          <div className="fixed top-0 right-0 h-full w-72 bg-white shadow-xl md:hidden z-50 p-5">
            <div className="flex items-center justify-between mb-6">
              <Image src="/logo-amarte.png" alt="AMARTE" width={100} height={32} />
              <button
            ...
            <nav className="flex flex-col gap-4 text-base">
              <Link href={`/${lang}`} onClick={() => setOpen(false)}>{dict.nav.home}</Link>
              <Link href={`/${lang}/about`} onClick={() => setOpen(false)}>{dict.nav.about}</Link>
              <Link href={`/${lang}/programs`} onClick={() => setOpen(false)}>{dict.nav.programs}</Link>
              <Link href={`/${lang}/impact`} onClick={() => setOpen(false)}>{dict.nav.impact}</Link>
              <Link href={`/${lang}/get-involved`} onClick={() => setOpen(false)}>{dict.nav.getInvolved}</Link>
